function New-ImageFromGithub {
    ...
    process {
        ...
        # Make a random folder to work in
        ...
        # First build a Linux AMD64 image
        $PSBoundParameters["OS"] = "linux|unix"
        $PSBoundParameters["Architecture"] = "amd64|x64|x86_64"

        $linux = SelectAssetByPlatform -Assets $release.assets @PSBoundParameters

        # Creates a named subdirectory for the package extraction
        $PackagePath = GetPackage $linux -Repo $release.Repo

        # Create another subdirectory for the package staging
        ...
        MoveExecutable -FromDir $PackagePath -ToDir $BinDir @PSBoundParameters -Repo $release.Repo
        | ForEach-Object {
            if ($IsLinux -or $IsMacOS) {
                chmod +x $_.FullName
            }
            $_
        } -OutVariable Executable | Out-Null
        ...
        # Then, build a Windows AMD64 image
        $PSBoundParameters["OS"] = "windows|(?<!dar)win"
        $PSBoundParameters["Architecture"] = "amd64|x64|x86_64"

        $windows = SelectAssetByPlatform -assets $release.assets  @PSBoundParameters
        # Results in a Push-Location to a temp folder
        $PackagePath = GetPackage $windows -Repo $release.Repo
        ...
        # Get-ChildItem $PackagePath -File
        # | FixName -Architecture "amd64|x64|x86_64" -OS "windows|(?<!dar)win" -Repo $release.Repo
        ...
        MoveExecutable -FromDir $PackagePath -ToDir $WinDir @PSBoundParameters -Repo $release.Repo
        | ForEach-Object {
            if ($IsLinux -or $IsMacOS) {
                chmod +x $_.FullName
            }
